...
export async function transferStock(payload: TransferPayload) {
  const { productId, variantId, fromLocationId, toLocationId, inventoryItemId, quantity, notes, user } = payload;

  if (!productId || !fromLocationId || !toLocationId || !inventoryItemId) {
    return { success: false, message: 'Product, lot, and both locations are required.' };
  }
...
export async function transferGodownStockAggregated(payload: {
  ...
  if (!productId || quantity <= 0) {
    return { success: false, message: 'Valid product and quantity > 0 are required.' };
  }
...
export async function transferReservedStockAggregated(payload: {
  ...
  if (!productId || !fromLocationId || quantity <= 0 || !Number.isInteger(quantity)) {
    return { success: false, code: 'INVALID_QUANTITY', message: 'Valid product, location, and integer quantity > 0 are required.' };
  }
