...
import { ChevronLeft, ShoppingCart, Play } from "lucide-react";
...
export default function ProductDetailClient({ product }: { product: Product }) {
  ...
  const addToCart = (variant?: Variant) => {
    ...
    const cart = JSON.parse(localStorage.getItem("wholesale_cart") || "[]");
    ...
    toast({
      title: "Added to cart",
      description: `${product.name}${variant ? ` (${variant.name})` : ""} x${minQty}`,
    });
  };
                  ...
                  <Card key={variant.id}>
                    <CardContent className="p-3 flex items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Button size="sm" onClick={() => addToCart(variant)}>
