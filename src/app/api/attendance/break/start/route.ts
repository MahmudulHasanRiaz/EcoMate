...
import { startBreakForStaff } from '@/server/modules/attendance';
...
export async function POST(req: Request) {
  try {
    ...
    const record = await startBreakForStaff(staffId);
    return NextResponse.json(record);
  } catch (error: any) {
    console.error('[API:ATTENDANCE_BREAK_START]', error);
    const message = error?.message || 'Failed to start break';
