...
import { startInactiveForStaff } from '@/server/modules/attendance';
...
export async function POST(req: Request) {
  try {
    ...
    const record = await startInactiveForStaff(staffId);
    return NextResponse.json(record);
  } catch (error: any) {
    console.error('[API:ATTENDANCE_INACTIVE_START]', error);
    const message = error?.message || 'Failed to start inactive';
