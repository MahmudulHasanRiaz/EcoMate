...
import { endInactiveForStaff } from '@/server/modules/attendance';
...
export async function POST(req: Request) {
  try {
    ...
    const record = await endInactiveForStaff(staffId);
    return NextResponse.json(record);
  } catch (error: any) {
    console.error('[API:ATTENDANCE_INACTIVE_END]', error);
    const message = error?.message || 'Failed to end inactive';
