...
export async function POST(req: NextRequest) {
    try {
        ...
        // Verify there is NO open session already
        const activeSession = await prisma.cashDrawerSession.findFirst({
            where: { cashDrawerId: showroom.cashDrawerId, status: 'Open' }
        });
        if (activeSession) {
            return apiError('Session already open for this drawer', 409, { code: 'SESSION_ALREADY_OPEN' });
        }
        ...
        console.error('[API:POS_DRAWER_SESSION_OPEN]', e);
